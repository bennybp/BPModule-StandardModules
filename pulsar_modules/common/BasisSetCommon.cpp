                                         ...
                                         const BasisSet & bs)
{
    ...
    const bool use_dist=false;
    std::string cachekey = std::string("bs:") + hash_to_string(bs.my_hash());
    auto ret=cache.get<BasisSet>(cachekey,use_dist);
    if(ret) // options are unimportant
    {
    ...
    // add to the cache
    cache.set(cachekey, std::move(newbs), CacheData::CheckpointLocal);

    // retrieve the shared_ptr from cache and return

    return cache.get<BasisSet>(cachekey,use_dist);
}
